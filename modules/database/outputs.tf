...
output "generated_admin_passwords" {
  description = "Autonomous Database admin passwords generated by Terraform (empty when supplied via config)"
  value = {
    atp = var.create_atp && var.atp_config.admin_password == "" ? random_password.atp_admin[0].result : null
    adw = var.create_adw && var.adw_config.admin_password == "" ? random_password.adw_admin[0].result : null
  }
  sensitive = true
}
