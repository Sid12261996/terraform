...
provider "oci" {
  # Configuration supports both instance principals (in OCI) and API keys (local/CI)
  # See variables.tf for authentication options
  region = var.oci_region
  
  # Instance principals - used when running in OCI (GitHub Actions self-hosted, OCI compute)
  # API keys - used for local development and GitHub-hosted runners
  # Both can be configured; provider uses whichever is available
}
...
# Get current tenancy and user info for reference
