...
output "deployment_summary" {
  ...
  value = {
    ...
    compartments     = output.compartment_ocids.value
    vcn              = output.vcn_id.value
    subnets = {
      public  = output.public_subnet_ocids.value
      private = output.private_subnet_ocids.value
    }
    gateways = {
      internet = output.internet_gateway_id.value
      nat      = output.nat_gateway_id.value
      service  = output.service_gateway_id.value
    }
    compute = {
      instances = output.instance_ocids.value
      private_ips = output.instance_private_ips.value
    }
    load_balancer = {
      public  = output.public_lb_id.value
      private = output.private_lb_id.value
    }
    database = {
      atp = output.atp_database.value
      adw = output.adw_database.value
      db_system = output.db_system.value
    }
    state_backend = {
      bucket = output.state_bucket_name.value
      lock_table = output.state_lock_table_name.value
    }
  }
}